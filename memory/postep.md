...
## Zrobione niedawno
- **Audyt zmiany email / reset hasła (2026-04-27)** — migracja `026_audit_log_auth_actions.sql` (CHECK `audit_log.action` + `auth_email_change` / `auth_password_reset`); `api/routes/residents.py` — `try`/`except` przy INSERT audytu (fallback: warning w logach, 200 dla UI jeśli audyt nie przejdzie). CHANGELOG, `operations/01`, CLAUDE / `.cursorrules`, `feature-map`.
