...
- Migracje 001-024 (m.in. 017 zarządca, 018 grupy rozliczeniowe, 019 billing_surname, 020 zgody RODO w `residents`, 021 `announcements.is_public`, 022 domyślna wartość `is_public`, 023 widok `last_import_activity` dla „Saldo na dzień" w panelu mieszkańca, 024 `resolutions.is_test` + `reminder_sent_at`) — uruchamiane przez SQL Editor w dashboardzie Supabase
