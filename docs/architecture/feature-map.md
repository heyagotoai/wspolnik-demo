...
| Mieszkańcy | `/admin/mieszkancy` | CRUD przez [[FastAPI]] (tworzenie/usuwanie) + Supabase (edycja). Auto-sync owner_resident_id przy tworzeniu/usuwaniu. Auto-scroll do formularza edycji. **Email opcjonalny** (migracja 025): brak email = mieszkaniec „bez konta" — placeholder auth user z banem ~100 lat, `has_account=false`, badge „bez konta" w tabeli; admin może nadać konto później (edycja: email + hasło → `ban_duration: none`, `has_account=true`). **Awaryjna zmiana danych logowania** (mieszkaniec z kontem): przycisk **„Zmień email"** obok pola email → `PATCH /residents/{id}/email` (audit `auth_email_change`, wymuszone wylogowanie wszystkich sesji); przycisk **„Wygeneruj nowe hasło"** → `POST /residents/{id}/reset-password` → modal pokazujący raz 12-znakowe losowe hasło (bez znaków mylących `0/O/1/l/I`) z akcją **Kopiuj** (admin przekazuje hasło mieszkańcowi; system nie zachowuje); audit `auth_password_reset` + global sign-out (**migracja 026** rozszerza CHECK `audit_log.action`, inaczej wstawienie audytu kończyło się błędem po udanej zmianie). **Zarządzanie lokalami** (modal 🏠): lista lokali właściciela z odpinaniem + dropdown wolnych lokali do przypisania (`POST/DELETE /residents/{id}/apartments`); kolumna „Lokal" wyświetla wszystkie lokale |
...
| Import z zestawienia bankowego (.xls) | ✅ done | WYSOKI | `POST /api/import/payments-bank-statement`; dopasowanie po `billing_surname` i numerach lokali z opisu/adresu; **ręczne przypisanie niedopasowanych** w podglądzie modalu (selektor 1+ lokali, split proporcjonalny lub równy) — pole formularza `manual_assignments` (JSON `{row_index: [apartment_id,...]}`); deduplikacja `(lokal, data)` — [[ADR-014-payment-import-deduplication|ADR-014]]; parser: `api/services/bank_statement_parser.py` |
