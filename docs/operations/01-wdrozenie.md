...
- Backend: `api/requirements.txt` (m.in. **`tzdata`** — na Windowsie bez systemowej bazy IANA wymagany do `Europe/Warsaw` w treści maila o saldzie; **`openpyxl`** — import z Excel w `/api/import`: stan początkowy lokali oraz wpłaty z arkusza dopasowań)
